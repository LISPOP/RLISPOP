...
We have stored a series of CSV files from the City of Toronto in the folder `course_data/parking`. 
...
You can also combine `here` with `..` to move up one directory!
...
From there, you can navigate down one directory. We can use the command `list.files()` to list the files in a directory

```{r here-data}
list.files(here("../course_data/parking"))
```
We just add that first file name and append it to the path name we have developed and pass it to `read.csv`
...
* folder - here you have to specify the folder that holds the CSV files you want to import. It's always best to use the `here()` command to get there. 
...
## Labelled Variables Versus Factors 
...
Notice underneath each variable name is printed `chr+lbl` or maybe `S3: `haven_labelled` for the first two variables and `dbl` for the third? 
The first two variables are `character` and `labelled` variables while the third is a `numeric` (`dbl` is just a numeric variable with a decimal point).
...
Now, you can't actually do much with variables that are stored as `labelled`. If you want to see what the distribution of responses is of the provinces, you can try `table()`
...
But we don't know here what 14 refers to. 
...
1. An easy way is to wrap a variable in `as_factor()` from the `labelled` package. It simply takes the labels and applies them to the numbers they are mapped to them and, done. However this only does this the one time. 
...
2. Other times, you want to make a more permanent solution. Consider the ``HHType6` variable. 
...
Not every example of converting `labelled` data will be like this, it will often be simpler, but I do want to highlight this issue when dealing with data that comes from SPSS or Stata.
