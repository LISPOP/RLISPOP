...
#| label: pivot-tidy
classroom %>% 
  pivot_longer(quiz1:test1, names_to="assessment", values_to=c("grade"))->classroom_tidy
classroom_tidy
...
#look for variables in the shs that have `income` in their value.
lookfor(shs,"income")
shs %>% 
  #Select all the variables that end with the phrase TotInc. Take prov as well and rename Province
  select(ends_with("TotInc")) %>% 
  #run as_factor for the province variable
  # store in shs_subset
  as_factor()->shs_subset
#glimpse
glimpse(shs_subset)


## -------------------------------------------------------------------------------------------
#| label: pivot-cols-by-position
shs_subset %>% 
  pivot_longer(data=., cols=1:4)


## -------------------------------------------------------------------------------------------
#| label: pivot-cols-by-name
shs_subset %>% 
  pivot_longer(data=., cols=RP_TotInc:HH_TotInc)


## -------------------------------------------------------------------------------------------
#| label: pivot-cols-by-select
shs_subset %>% 
  pivot_longer(data=., cols=ends_with("Inc"))->shs_subset_long
#Check it out. 
head(shs_subset_long)
...
#| label: specify-names
shs_subset %>% 
  pivot_longer(data=., cols=ends_with("Inc"), names_to=c("Income"), values_to=c("Dollars"))->shs_subset_long
