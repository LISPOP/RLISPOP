...
```{r remove-df-function, echo=F, results='hide', purl=F}
df<-NULL
df
```
...
However, we can save them by assigning the results to another short-hand object with the `<`
...
Now remember when I talked about how the *script* is real? Not the workspace?  One consequence of that is that we can, and often do, *clear the contents of the workspace* and start again by running our code. 

You can do that easily with the little broom icon on the top right. 


```{r img3, echo=F, out.width='100%', purl=F}
library(knitr)
...
```{r make-fake-data, echo=F, purl=F}
#first name
first_name<-c('Simon')
#last name
last_name<-c('Kiss')
...
#date of birth
dob<-c('1976-08-14')
df<-data.frame(first_name, last_name, sex, age, dob)
df2<-df
```
...
Now, we actually want that data frame back. So, go back into your script and comment out the command `rm(df)` with a `#` in front of it. Now, execute your code again from the top to the bottom. 
